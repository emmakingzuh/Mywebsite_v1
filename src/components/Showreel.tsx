...
export function ShowreelModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] grid place-items-center bg-black/80 p-4 backdrop-blur-md" onClick={onClose}>
      <button
        className="absolute right-5 top-5 grid h-11 w-11 place-items-center rounded-xl glass text-white"
        onClick={onClose}
        aria-label="Close"
      >
        <X size={20} />
      </button>
      <div className="aspect-video w-full max-w-4xl overflow-hidden rounded-3xl" onClick={(e) => e.stopPropagation()}>
        <iframe
          src={`https://player.vimeo.com/video/${reel.embedId}?autoplay=1&title=0&byline=0&portrait=0`}
